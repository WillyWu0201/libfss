...
func main() {
	...
	// Test with if x = 10, evaluate to 2   5,1
	...
	// fmt.Println("9 is No Match (should be 0):", ans0+ans1)

	// Test 2-party Less than Function
	// Test if x < 10, evaluate to 2
	// fssKeysLt := fClient.GenerateTreeLt(5, 1)

	// var anslt0, anslt1 uint = 0, 0
	// anslt0 = fServer.EvaluateLt(fssKeysLt[0], 3)
	// anslt1 = fServer.EvaluateLt(fssKeysLt[1], 3)
